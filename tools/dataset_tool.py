...
class DatasetSearchTool(BaseTool):
    ...
    description: str = "Search datasets on Kaggle, HuggingFace, GitHub"
    ...
    def _search_github(self, query: str) -> List[Dict]:
        try:
            ...
            resp = requests.get(url, params={"q": f"{query} dataset", "sort": "stars"}, timeout=5)
            results = []
            if resp.status_code == 200:
                for item in resp.json().get("items", [])[:3]:
                    results.append({
                        ...
                        "url": item["html_url"],
                        "description": (item.get("description", ""))[:100] + "...",
                        "quality": f"{min(10, item.get('stargazers_count', 0)//100)}/10"
                    })
            return results
        except:
            return [{
